...
def gerarMigration():
    ...
    os.system('git pull origin {ambiente}')
      ...
      for tipoNota in arrayTipoNota:
            if request.form.get(f'checkbox_card_{tipoNota}') != None:
                ...
                if request.form.get(f'intervalo_notas_{tipoNota}') == 'todas':
                    ...
                    codigo_down += codigoDownTodas(tipoNotaUpper)
      return f"""
...
def gerarArquivoMigration():
    ...
    if ambiente == 'develop':
        os.system('git checkout -b feature-leg-dev-{numero_card}')
    else:
        os.system('git checkout -b feature-leg-{numero_card}')
    ...
    os.system('git add {ultimo_arquivo}')
    os.system('git commit -m "Exclusão de notas do cliente {id_cliente}"')
    if ambiente == 'develop':
        os.system('git push origin feature-leg-dev-{numero_card}')
    else:
        os.system('git push origin feature-leg-{numero_card}')
...
def codigoUpTodas(tipoNotaUpper):
     return f"""
        DB::table(self::{tipoNotaUpper}_TABLE)
            ->where('id_empresa', '=', self::CLIENTE)
            ->update(['lixeira' => 'Sim']);
...
def codigoDownTodas(tipoNotaUpper):
    return f"""
        DB::table(self::{tipoNotaUpper}_TABLE)
            ->where('id_empresa', '=', self::CLIENTE)
            ->update(['lixeira' => 'Nao']);          
